...
router.get("/stats", firebase_1.requireFirebase, async (req, res) => {
    try {
        ...
        const standardUsers = users.filter((user) => user.accessLevel === "standard").length;
        const premiumUsers = users.filter((user) => user.accessLevel === "premium").length;
        const adminUsers = users.filter((user) => user.accessLevel === "admin").length;
...
router.get("/recent-users", firebase_1.requireFirebase, async (req, res) => {
    try {
        const db = (0, firebase_1.getFirebaseDb)();
